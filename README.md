...
这是一套由 5 个独立专业 Skill 和 1 个轻量发现入口组成的 V1。五个专业模块共享结构化、可追溯、手机低摩擦的原则，但各自只完成一种任务。入口会查看当前可用的个人 Skills：意图明确时直接调用一个并告知用户，意图含糊时只问一个确认问题；它不会自动串联模块，因此当前版本仍不包含总控 Agent。
...
当用户没有说明想怎样处理一段文字或图片时，`$choose-personal-workflow-v1` 会发现最合适的已安装个人 Skill，并问一句确认问题。它也会识别套件之外的个人 Skill，例如“内容雷达”。用户不需要记住任何英文名称。
